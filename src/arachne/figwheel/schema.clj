...
(def schema
  ...
  (concat
    ...
    (m/type :arachne.figwheel/Server [:arachne/Component]
      "A figwheel server"
      (m/attr :arachne.figwheel.server/root :one :string
        "The directory from which files will be served via HTTP")
      (m/attr :arachne.figwheel.server/port :one :long
        "The HTTP port that figwheel will use")
      ...
      (m/attr :arachne.figwheel.server/builds :many :arachne.figwheel/Build
        "The builds used by this server"))

    (m/type :arachne.figwheel/Build []
      "A ClojureScript build definition"
      (m/attr :arachne.figwheel.build/id :one :string
        "The string name of the build")
      (m/attr :arachne.figwheel.build/source-paths :one-or-more :string
        "Paths that contain CLJS source")
      (m/attr :arachne.figwheel.build/figwheel :one :boolean
        "Is this build figwheel enabled?")
      (m/attr :arachne.figwheel.build/compiler-options :one :ref :arachne.cljs/CompilerOptions
        "ClojureScript build options for this build"))

    ;; NOTE: The following are general CLJS options and should probably be split off to a separate CLJS module
